...
public class StructureManager {
    ...
    private Path getContentBasePath() {
        return Paths.get(GLOBAL_CONTENT_FOLDER, languageCode);
    }
    private Path getDefaultContentPathBase() {
        return Paths.get(GLOBAL_CONTENT_FOLDER, defaultLanguage);
    }

    // return full content path
    // check if file exist within locale
    // if not, return default locale file
    private Path getContentPathOrDefault(Path endPath) {
        Path contentPath = getContentBasePath().resolve(endPath);
        File contentFile = new File(String.valueOf(contentPath));
        if (!contentFile.exists()){
            contentPath =  getDefaultContentPathBase().resolve(endPath);
        }
        return contentPath;
    }
    ...
    public String getTaskDescriptionForModelPath(String modelId, String taskId) {
        Path endPath = Paths.get("model",modelId,"task",taskId+".md");
        return String.valueOf(getContentPathOrDefault(endPath));
    }

    public String getExampleDescriptionPath(String exampleId) {
        Path endPath = Paths.get("example",exampleId + ".md");
        return String.valueOf(getContentPathOrDefault(endPath));
    }
